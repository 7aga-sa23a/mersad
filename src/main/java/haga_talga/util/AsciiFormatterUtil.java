 ...
 *    AsciiFormatterUtil.box("Menu item 1\nMenu item 2", "magenta", "rounded", "center", 80);
...
public class AsciiFormatterUtil {
    ...
    private static List<String> drawBox(String[] contentLines, BoxChars bc, int padding, String contentAlign) {
        int maxLen = 0;
        for (String l : contentLines) maxLen = Math.max(maxLen, visibleLength(l));
        ...
        for (String line : contentLines) {
            ...
            int left = 0;
            if ("center".equalsIgnoreCase(contentAlign)) left = pad / 2;
            else if ("right".equalsIgnoreCase(contentAlign)) left = pad;
            int right = pad - left;
    ...
    private static String alignLine(String line, String align, int width) {
        if (align == null || align.equalsIgnoreCase("left")) return line;
        int len = visibleLength(line);
        if (len >= width) return line;
        if (align.equalsIgnoreCase("center")) return " ".repeat((width - len) / 2) + line;
        if (align.equalsIgnoreCase("right"))  return " ".repeat(width - len) + line;
        return line;
    }
    ...
    private static String[] renderAsciiArt(String text) {
        ...
        for (int r = 0; r < rows; r++) lines[r] = new StringBuilder();
        for (int i = 0; i < charRows.length; i++) {
            String[] glyph = charRows[i];
            for (int r = 0; r < rows; r++) {
                lines[r].append(glyph[r]);
                if (i < charRows.length - 1) lines[r].append(" ");
            }
        }
        String[] result = new String[rows];
        for (int r = 0; r < rows; r++) result[r] = lines[r].toString();
        return result;
    }
     ...
     * @param color "cyan" | "red" | "green" | "yellow" | "blue" | "magenta" | "white"
    ...
    /**
     * Btetba3 prompt bel color 3ashan ta5od input mn el user (el cursor byf4al f nafs el satr).
    ...
    public static void table(String[] headers, String[][] rows, String color) {
        ...
        for (int i = 0; i < colWidths.length; i++) {
            ...
            if (i < colWidths.length - 1) { top.append("┬"); mid.append("┼"); bot.append("┴"); }
            else                          { top.append("┐"); mid.append("┤"); bot.append("┘"); }
        }
        System.out.println(c + top + RESET);
        printRow(headers, colWidths, c);
        System.out.println(c + mid + RESET);
        for (String[] row : rows) printRow(row, colWidths, c);
